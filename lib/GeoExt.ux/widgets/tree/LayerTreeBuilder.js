...
GeoExt.ux.tree.LayerTreeBuilder = Ext.extend(Ext.tree.TreePanel, {
	...
	otherLayersText: "Outras camadas",

	/** api: config[baseLayersText] ``String`` i18n */
	baseLayersText: "Cartografia de Base",
	/* end i18n */
			...
			if (groups.length == 0) {
				...
				group == this.otherLayersText) {
					...
					createNode = function(attr) {
						...
						// jgr
						attr.iconCls = layer.iconCls;
						attr.icon = layer.icon;
						attr.qtip = layerRecord.get("layer").qtip;
